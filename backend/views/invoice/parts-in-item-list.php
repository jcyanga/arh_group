...
<?php if(!empty($getParts)): ?>
    ...
    <?php foreach($getParts as $row): ?>
        ...
        <div class="parts-item-<?= $row['id'] ?>">
            ...
            <div class="col-md-3">
                ...
                <input type="text"  id="parts-item-qty-<?= $row['id'] ?>" onchange="invUpdatePartsSubTotal(<?= $row['id'] ?>)" class="quantity form_quoSP form-control" placeholder="0" />
            ...
            <div class="col-md-3">
                ...
                <input type="text" id="parts-item-unitPrice-<?= $row['id'] ?>" onchange="invUpdatePartsSubTotal(<?= $row['id'] ?>)" class="unit_price form_quoSP form-control" value="<?= $row['selling_price'] ?>" placeholder="0.00" />
                <br/><br/>
    ...
    <?php endforeach; ?>
        ...
        <div class="quoSPAction pull-right">
            <a href="javascript:invAddPartsToSelectedItem(<?= $row['id'] ?>)" id="quoAddItemList" class="form-btn btn btn-info btn-lg" ><i class='fa fa-cart-plus'></i> Add Product in List </a>
        </div>
        </div>

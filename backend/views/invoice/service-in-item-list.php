...
<?php if(!empty($getServices)): ?>
    ...
    <?php foreach($getServices as $row): ?>
        ...
        <div class="service-item-<?= $row['id'] ?>">
            ...
            <div class="col-md-3">
                ...
                <input type="text"  id="service-item-qty-<?= $row['id'] ?>" onchange="invUpdateServiceSubTotal(<?= $row['id'] ?>)" class="quantity form_quoSP form-control" value="1" placeholder="Qty" />
                <br/>
            ...
            <div class="col-md-3">
                ...
                <input type="text" id="service-item-unitPrice-<?= $row['id'] ?>" onchange="invUpdateServiceSubTotal(<?= $row['id'] ?>)" class="unit_price form_quoSP form-control" value="<?= $row['default_price'] ?>" placeholder="0.00" />
                <br/>
    ...
    <?php endforeach; ?>
        ...
        <div class="quoSPAction pull-right">
            <a href="javascript:invAddServiceToSelectedItem(<?= $row['id'] ?>)" id="quoAddItemList" class="form-btn btn btn-info btn-lg" ><i class='fa fa-cart-plus'></i> Add Service in List </a>
        </div>
        </div>

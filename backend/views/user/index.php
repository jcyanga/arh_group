...
<div class="form-container user-index">
...
<?php } ?>
    ...
    <div class="other-btns-container">
        <p>
            <?= Html::a('<i class=\'icon-plus-sign\'></i> New User', ['create'], ['class' => 'form-btn btn btn-success']) ?>

            <?= Html::a('<i class=\'icon-print\'></i> Export User List', '?r=customer/status', ['onclick' => 'return print_confirmation()', 'class' => 'form-btn btn btn-warning']) ?>
             ...
             [
                ...
                [
                    ...
                    'urlCreator' => function ($action, $model, $key, $index) {
                        if ($action === 'preview') {
                            $url ='?r=user/view&id='.$model->id;
                            return $url;
                        }
                        if ($action === 'amend') {
                            $url ='?r=user/update&id='.$model->id;
                            return $url;
                        }
                        if ($action === 'remove') {
                            $url ='?r=user/delete-column&id='.$model->id;
                            return $url;
                        }
                        if ($action === 'history') {
                            $url ='?r=user/history&id='.$model->id;
                            return $url;
                        }
                        if ($action === 'status') {
                            $url ='?r=user/status&id='.$model->id;
                            return $url;
                        }
                    }
                ],
